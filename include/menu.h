...
namespace case_handling{
    ...
    int make_prompt_int(tui::string &msg, std::pair<unsigned, unsigned> start_coords = {tui::cursor::get_position().first, tui::cursor::get_position().second});
    std::string get_valid_name(shop &shop, tui::string &msg, std::pair<unsigned, unsigned> start_coords);
    int get_valid_amount(shop &shop, std::string &product_name, tui::string &msg, std::pair<unsigned, unsigned> start_coords);
};
struct mapper{
