...
void shop::add_product(std::string name, double price,
                                unsigned int instock) {
  product new_product(name, price, instock);
  this->products.push_back(new_product);
}

void shop::delete_product(std::string name) {
  this->products.erase(this->products.begin() + this->binary_search_product_index(name));
}
...
void shop::sell(std::string from, unsigned int amount) {
  this->products.at(this->binary_search_product_index(from))
      .set_instock(this->products.at(this->binary_search_product_index(from)).get_instock() -
                   amount);
  // we look for the product in the vector with the binary search method
  // than we set the instock amount to the difference of the current value and the sold amount
}
void shop::restock(std::string from, unsigned int amount) {
  this->products.at(this->binary_search_product_index(from))
      .set_instock(this->products.at(this->binary_search_product_index(from)).get_instock() +
                   amount);
  // we look for the product in the vector with the binary search method
  // than set the instock amount to the sum of the current value and the restocked amount
}
...
void shop::list_specific_product(std::string name) {
  ...
  product wanted_product = this->products.at(this->binary_search_product_index(name));
  // std::cerr << "found\n";
  std::cout << wanted_product.get_name() << " - " << wanted_product.get_price()
...
void shop::load_data() {
  ...
  while (getline(fin, name)) {
    ...
    fin.get(); // needed to skip '\n' character at the end of the "instock" number
