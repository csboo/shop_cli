...
void shop::sell(std::string from, unsigned int amount) {
    this->products.at(this->binary_search_product_index(from))
        .set_instock(this->products.at(this->binary_search_product_index(from)).get_instock() - amount);
...
void shop::add_product(const std::string& name, double price, unsigned int instock) {
    ...
    std::vector<product>::iterator iter = std::lower_bound(
        this->products.begin(), this->products.end(), sajat_to_lower(name),
        [](const product& p, const std::string& name) { return sajat_to_lower(p.get_name()) < sajat_to_lower(name); });
...
void shop::list_products() {
    ...
    for (auto arucikk : this->products) {
        std::cout << "\n";
        line =
            concat(arucikk.get_name(), " - ", arucikk.get_price(), "Ft,    Raktaron: ", arucikk.get_instock(), " db");
...
void shop::list_specific_product(std::string& name) {
    ...
    // std::cerr << "found\n";
    line = concat(wanted_product.get_name(), " - ", wanted_product.get_price(),
