...
int shop::binary_search_product_index(std::string& wanted_product) {
    ...
    while (begin <= end && index == -1) {
        ...
        if (sajat_to_lower(this->products.at(mid).get_name()) == sajat_to_lower(wanted_product)) {
            index = mid;
...
void shop::sort_products_by_name() {
    ...
    for (size_t i = 0; i < this->products.size() - 1; i++) {
        ...
        for (size_t j = i + 1; j < this->products.size(); j++) {
            if (sajat_to_lower(this->products.at(j).get_name()) < sajat_to_lower(this->products.at(min).get_name())) {
                min = j;
            }
        }
